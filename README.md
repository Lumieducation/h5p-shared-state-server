...
An experimental, ShareDB-based server component that adds multi-user
(real-time shared state) capabilities to
...
previously developed inside that monorepo and now lives here as its own
package with an independent release cycle.

## Installation

```bash
npm install @lumieducation/h5p-shared-state-server
```

It requires `@lumieducation/h5p-server` (from h5p-nodejs-library) as a peer
in your application, since it reads library and content metadata through the
same storage abstractions.

## Example

See [`example/`](./example) for a runnable Express server that wires up
`SharedStateServer` alongside `@lumieducation/h5p-server` and
